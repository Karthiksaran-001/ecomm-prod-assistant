...
class AgenticRAG:
    ...
    def _ai_assistant(self , state:AgentState):
        ...
        if any(word in last_message.lower() for word in ["price" , "reviews" , "product"]):
            return {"messages" : [HumanMessage(content= "TOOL : retriever")]}
        else:
            prompt = ChatPromptTemplate.from_template(
                "You are the Helpful AI Assitant. Answer the user directly/\n\nQuestion : {question}"
            )
    ...
    def _vector_retriver(self, state:AgentState):
        ...
        retriver = self.retriever_obj.load_retriever()
    ...
    def _build_workflow(self):
        ...
        workflow.add_node("Retriever", self._vector_retriver)
